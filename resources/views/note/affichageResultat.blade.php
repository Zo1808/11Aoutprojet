    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="row">
                ...
                RESULTAT DES EXAMENS DE {{App\Classes::select('nom_classe')->where('id',$classe_id)->first()->nom_classe}} ({{App\TypeExamens::select('type')->where('id',$type_exam_id)->first()->type}}-{{$type1}})<br/>
                    FILIERE : {{App\Filieres::select('nom')->where('id',$filiere_id)->first()->nom}}
                </center>
                <br/>
                <br/>
                <br/>
            ...
            <div class="adv-table">
              <table cellpadding="0" cellspacing="0" border="1" class="display table table-bordered">
                <thead>
                  ...
                  <th></th>
                    <th >UE</th>
                    @foreach(App\NoteRestit::join('unite_enseignements','unite_enseignements.id','note_restits.UE_id')->select('unite_enseignements.*')->where('note_restits.classe_id',$classe_id)->where('note_restits.filiere_id',$filiere_id)->distinct('unite_enseignement.UE')->get() as $item)
                    <th class="cell rotated_vertical_td" style="width:3%;"> <div class="rotated_vertical">{{$item->UE}}</div></th>
                     @endforeach
                    
                  </tr>
                  <tr>
                  <th style="width:1px;">Rang</th>
                  <th>Nom(s) et Prénom(s)</th>
                  @foreach(App\NoteRestit::join('unite_enseignements','unite_enseignements.id','note_restits.UE_id')->select('unite_enseignements.id')->where('note_restits.classe_id',$classe_id)->where('note_restits.filiere_id',$filiere_id)->distinct('unite_enseignement.UE')->get() as $item)
                ...
                <tr>
                   <td>{{$item['rang']}}</td>
                    <td>{{$item['nom_prenom']}}</td>
                                       @foreach(App\ResultatFinal::select('*')->where('classe_id',$classe_id)->where('filiere_id',$filiere_id)->where('type_exam_id',$type_exam_id)->where('etudiant_id',$item['etudiant_id'])->get() as $item1)
                                       @if($item1->moyenne > 10)
                                       <td>V</td>
                                       @else
                                       <td>NV</td>
              ...
              <tfooter>
              <form action="{{route('ExportPDFResultatGlobal')}}" method="POST">
              {{csrf_field()}}
             ...
             <hr>
                <button  class="pull-right"><i class="fa fa-print"></i></button>
                </form>
                </tfooter>
            </div>
